...
df1['Shot Result Player A'] = df1.apply(lambda row: determine_shot_result(row, 'Player A'), axis=1)
...
player_a_data = prepare_plot_data(df1, 'Shot Result Player A', shot_result_order)
all_categories = pd.DataFrame({'Rally Shot Type': player_a_data['Rally Shot Type'].unique()})
all_categories['Rally Shot Type'] = all_categories['Rally Shot Type'].astype(str)
player_a_data = pd.merge(all_categories, player_a_data, on='Rally Shot Type', how='left')
player_a_data['Count'] = player_a_data['Count'].fillna(0).astype(int)
player_a_data['Shot Result'] = player_a_data['Shot Result'].astype(str)
player_a_data['Shot Result'] = pd.Categorical(player_a_data['Shot Result'], categories=shot_result_order, ordered=True)
player_a_data = player_a_data.sort_values(by='Rally Shot Type', ascending=True)
bar = (
...
for result in shot_result_order:
    ...
    for cat in all_categories['Rally Shot Type']:
        row = player_a_data[(player_a_data['Rally Shot Type'] == cat) & (player_a_data['Shot Result'] == result)]
        if not row.empty:
            y_data.append({
