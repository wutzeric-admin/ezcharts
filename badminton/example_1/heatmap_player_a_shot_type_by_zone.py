...
# Filter df1 for Player A's shots
df1 = df[
    df['Primary category'].str.contains('Player A', na=False) &
...
for shot_type in shot_types:
    ...
    row_text = []
    for zone in zones:
        # Get all shots of this type in this zone from Player A's shots
        ...
        if count > 0:
            # Count immediate winners (won by player A)
            winners = sum(shots_df['Won By'].str.contains('player a', case=False, na=False))
            
            # Count immediate errors (won by opponent)
            errors = sum(shots_df['Won By'].str.contains('player b', case=False, na=False))
            ...
            for idx in continuing_shot_indices:
                ...
                if next_idx in df_full.index:
                    ...
                    if pd.notna(next_shot['Won By']):
                        if 'player a' in str(next_shot['Won By']).lower():
                            winner_setup += 1
                        elif 'player b' in str(next_shot['Won By']).lower():
                            weak_shots += 1
...
# Layout updates
fig.update_layout(
    title="Player A: Shot Types by Court Zone (Heatmap, Serves Removed)",
