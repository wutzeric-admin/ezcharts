...
fig = make_subplots(
    ...
    subplot_titles=[
        f"<b>{title}</b>" for title, zone in zip(zone_titles, zones_b)
    ]
)
...
for i, (zone, title) in enumerate(zip(zones_b, zone_titles)):
    row, col = divmod(i, 3)
    row += 1
    subset = player_b_data[player_b_data['Primary category'] == zone]
...
fig.update_layout(
    ...
    title=dict(
        text='Player B Hitting "From" Zones (Radius Scaled by Total Shots)',
